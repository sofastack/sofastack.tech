...
## 3. 如何构建 Spring Boot 宿主应用
Spring Boot 宿主应用和 Spring Boot 普通应用没有任何区别，打包插件保持不变。宿主应用包含多个业务应用的通用依赖（包括 SofaArk 依赖）。此处，我们首先介绍依赖配置，然后介绍打包。Spring Boot 宿主应用为业务应用提供通用逻辑和环境。首先我们介绍Ark相关的依赖配置，然后以样例代码介绍通用依赖。
...
### 4.2 打包运行
Spring Boot 宿主应用和 Spring Boot 普通应用没有任何区别，打包插件保持不变（`spring-boot-maven-plugin`）。打包运行先使用`spring-boot-maven-plugin`打包 为 Fat Jar，然后通过命令行运行。首先，使用构建配置如下：
```xml
<build>
    <plugins>
        <plugin>
